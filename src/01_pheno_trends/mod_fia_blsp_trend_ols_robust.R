...
# and with robust stand errors to account for heteroskedasticity. 
# 
# Author: Xiaojie Gao
# Date: 2024-10-03
# ******************************************************************************
...
SiteSpecTrend <- function(specname) {
    ...
    trend_dt <- lapply(spec_fit, function(thefit) {
        ...
        if (nrow(sos_noNA) > 10) {
            ...
            robo_se <- coeftest(lmfit$fit, vcov = vcovHC(lmfit$fit, "HC1"))
            spr_pval <- robo_se[2, 4]
            spr_slp <- robo_se[2, 1]
        } else {
            spr_pval <- NA
            spr_slp <- NA
...
fwrite(site_trend_dt, "pipe/fia_dom_blsp_trend_ols_robust.csv")
