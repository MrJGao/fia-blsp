...
SiteTrend <- function(spr_dm, tavg_name) {
    ...
    trend_dt <- lapply(ids, function(id) {
        ...
        # ~ Spring ----------------------------------------
        tavg <- thefit[, .(year, get(tavg_name))]

        lmfit <- base$FitLm(tavg$year, tavg$V2)
        pval <- lmfit$pval
        slp <- lmfit$cf[2]
...
# ~ Summer ----------------------------------------
smr_dm <- dm_dt[mth %in% c(6, 7, 8), .(
    smr_Tavg = mean((`tmax (deg c)` + `tmin (deg c)`) / 2)
), by = c("ID", "year")]

smr_dm_trend <- SiteTrend(smr_dm, tavg_name = "smr_Tavg")
...
temp_trend <- rbind(
    ...
    smr_dm_trend[, season := "smr"],
    atm_dm_trend[, season := "atm"],
    ann_dm_trend[, season := "ann"]
)
# out: fia_domspec_daymet_trend.csv
fwrite(temp_trend, "pipe/fia_domspec_daymet_temp_trends_ols.csv")
...
# out: fia_domspec_daymet_trend.csv
fwrite(prcp_trend, "pipe/fia_domspec_daymet_prcp_trends_ols.csv")
...
# out: fia_domspec_daymet_trend.csv
fwrite(srad_trend, "pipe/fia_domspec_daymet_srad_trends_ols.csv")
...
# out: fia_domspec_daymet_trend.csv
fwrite(vp_trend, "pipe/fia_domspec_daymet_vp_trends_ols.csv")
